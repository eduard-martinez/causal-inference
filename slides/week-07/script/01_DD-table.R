...
f_reg <- function(name_file,label,year_ini,year_end){
         ...
         mean_dep_var <- df %>% subset(year %in% year_ini:year_end) %>%
                         subset(sample_control_1==1 & d_control_1==1 & treat_staggered==0) %>%
         ...
         stargazer(m1_nb , m2_nb , m1_fxs , m2_fxs ,
                   ...
                   out = paste0("../../Aplicaciones/Overleaf/FxS_Crops/tables/main-regressions/main-reg_",name_file,"_",year_ini,"_",year_end,".tex"))
}
...
## Regression
f_reg(label="reg:main_reg" , name_file="crops" , year_ini=2011 , year_end=2021)
 
## tabla en espanol
table_eng <- read_csv(file="../../Aplicaciones/Overleaf/FxS_Crops/tables/main-regressions/main-reg_crops_2011_2021.tex" , col_names=F) %>% 
             unlist() %>% as.character()
...
table_eng[5] <- "\\label{reg:main_reg} \\vspace{-5mm}"
